    ...
    const identifyImage = async (additionalPrompt : string = "") => {
        ...
        try {
            ...
            const result = await model.generateContent([
                `Identify this image and provide its name and important information including a brief explanation about that image, First of all identify the provided image contains foods or not, then say the provided image do not contain any food if it contains then ignore this, if provided image contains foods then provide, calories, fat, carbs, protein, nutrition, energy, fibre, etc, the food contains in bullet-point format. ${additionalPrompt}`,
                imageParts,
            ]);
    ...
    <main className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12'>
        <div className='bg-white rounded-lg shadow-xl overflow-hidden'>
            <div className="p-8">
                ...
                <button type='button' onClick={() => identifyImage()} disabled={!image || loading} className='w-full bg-purple-600 text-white py-2 px-4 rounded-lg hover:bg-purple-700 transition duration-150 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed font-medium text-lg'>
